var app = angular.module('steem.services', []);
  ...
  app.directive('autofocus', ['$timeout',
    ...
    function ionComment() {
        return {
            ...
            controller: function($scope, $rootScope, $state, $ionicModal, $ionicPopup, $ionicActionSheet, $cordovaCamera, $filter, textAngularManager) {
                  ...
                  $scope.reply = function (xx) {
                    if (!$scope.editc) {
                        ...
                        if ($rootScope.$storage.user) {
                          ...
                          if (loginSuccess) {
                            $scope.data.comment = $scope.data.comment.replaceAll('style="text-align: right;"', 'class="text-right"');
                            $scope.data.comment = $scope.data.comment.replaceAll('style="text-align: left;"', 'class="text-left"');
                            $scope.data.comment = $scope.data.comment.replaceAll('style="text-align: center;"', 'class="text-center"');
    ...
    function ius($q, $ionicLoading, $cordovaFileTransfer, $ionicPlatform, $filter, $http) {
        var service = {};
        ...
        function uploadImage(imageURI) {
          ...
          var url = 'http://192.158.29.1:8080/api/upload';
          //var fd = new FormData();
          //fd.append("file", fs.createReadStream(imageURI));
          //console.log(fd);
          /*$http.post(url, imageURI, {transformRequest: angular.identity, headers: {'Content-Type': undefined} }).success(function(d){
            console.log(d);
            deferred.resolve(d);
          }).error(function(err){
            console.log(err)
          ...
          request.post({url:url, formData: imageURI}, function optionalCallback(err, httpResponse, body) {
            if (err) {
          ...
          xhr.open('POST', 'http://192.158.29.1:8080/api/upload', true);
