      ...
      if (!$rootScope.$storage.user) {
        ...
        window.Api.database_api().exec("get_accounts", [[$scope.loginData.username]]).then(function(dd){
          ...
          if (!loginSuccess) {
            ...
            $rootScope.$storage.subscription = {vote:false, follow:true, comment:true, mention:true, resteem:true};
...
app.controller('PostsCtrl', function($scope, $rootScope, $state, $ionicPopup, $ionicPopover, $interval, $ionicScrollDelegate, $ionicModal, $filter, $stateParams, $ionicSlideBoxDelegate, $ionicActionSheet, $ionicPlatform, $cordovaCamera, ImageUploadService, $filter, $ionicHistory, $timeout, textAngularManager) {
  ...
  $scope.insertImage = function(type) {
    ...
    if (type == 0) {
      ...
      remote.dialog.showOpenDialog(remote.getCurrentWindow(), { title: 'Upload Attachments', buttonLabel: 'Upload', filters: [ { name: 'Images', extensions: ['jpg', 'png', 'jpeg', 'gif'] } ], properties: ['openFile'] }, function(imageData){
      if (imageData) {
        var fd = {
          field_: 'value_',
...
app.controller('SettingsCtrl', function($scope, $stateParams, $rootScope, $ionicHistory, $state, $ionicPopover, $ionicPopup, APIs, $filter, $translate, $window) {
  ...
  $scope.$on('$ionicView.beforeEnter', function(){
    ...
    if ($rootScope.$storage.user) {
      ...
      angular.merge($scope.data, {vote: $rootScope.$storage.subscription.vote, follow: $rootScope.$storage.subscription.follow, comment: $rootScope.$storage.subscription.comment, mention: $rootScope.$storage.subscription.mention, resteem: $rootScope.$storage.subscription.resteem});
      if (!$scope.$$phase){
        $scope.$apply();
      }
    }
