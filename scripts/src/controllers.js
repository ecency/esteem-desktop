var app = angular.module('steem.controllers', []);

app.controller('AppCtrl', function ($scope, $ionicModal, $timeout, $rootScope, $state, $ionicHistory, $cordovaSocialSharing, ImageUploadService, $cordovaCamera, $ionicSideMenuDelegate, $ionicPlatform, $filter, APIs, $window, $ionicPopover, $ionicPopup) {

    $scope.loginData = {};

    $ionicModal.fromTemplateUrl('scripts/templates/login.html', {
        scope: $scope
    }).then(function (modal) {
        $scope.loginModal = modal;
    });

    $ionicPopover.fromTemplateUrl('scripts/templates/popover.html', {
        scope: $scope,
                ...
                {text: 'Cancel'},
                {
                    ...
                    onTap: function (e) {
                        if (!$scope.data.text) {
                            ...
                            require('electron').clipboard.writeText($scope.data.text);
        ...
        if ($rootScope.$storage.user && $rootScope.$storage.user.username) {
            if (typeof window.Api.database_api === "function") {
                window.Api.database_api().exec("get_accounts", [[$rootScope.$storage.user.username]]).then(function (dd) {
                    ...
                    $scope.mcss = ($rootScope.$storage.user.json_metadata && $rootScope.$storage.user.json_metadata.profile.cover_image) ? {
                        'background': 'url(' + $rootScope.$storage.user.json_metadata.profile.cover_image + ')',
    ...
    $ionicPlatform.ready(function () {
        if (window.cordova) {
            ...
            $rootScope.$storage.appversion = '1.0.0';
        }
    });
    ...
    $scope.logout = function () {
        ...
        $ionicSideMenuDelegate.toggleLeft();
        $window.location.reload(true);
        $rootScope.$storage.subscription = {vote: false, follow: false, comment: false, mention: false, resteem: false};
    ...
    $scope.data = {};
    $ionicModal.fromTemplateUrl('scripts/templates/search.html', {
        scope: $scope,
...
app.controller('PostsCtrl', function ($scope, $rootScope, $state, $ionicPopup, $ionicPopover, $interval, $ionicScrollDelegate, $ionicModal, $filter, $stateParams, $ionicSlideBoxDelegate, $ionicActionSheet, $ionicPlatform, $cordovaCamera, ImageUploadService, $filter, $ionicHistory, $timeout, textAngularManager) {


    $scope.activeMenu = $rootScope.$storage.filter || "trending";
    $scope.mymenu = $rootScope.$storage.user ? [{
        text: $filter('translate')('FEED'),
        custom: 'feed'
    }, {text: $filter('translate')('TRENDING'), custom: 'trending'}, {
        text: $filter('translate')('HOT'),
        custom: 'hot'
    }, {text: $filter('translate')('NEW'), custom: 'created'}, {
        text: $filter('translate')('ACTIVE'),
        custom: 'active'
    }, {text: $filter('translate')('PROMOTED'), custom: 'promoted'}, {
        text: $filter('translate')('TRENDING_30'),
        custom: 'trending30'
    }, {text: $filter('translate')('VOTES'), custom: 'votes'}, {
        text: $filter('translate')('COMMENTS'),
        custom: 'children'
    }, {text: $filter('translate')('PAYOUT'), custom: 'cashout'}] : [{
        text: $filter('translate')('TRENDING'),
        custom: 'trending'
    }, {text: $filter('translate')('HOT'), custom: 'hot'}, {
        text: $filter('translate')('NEW'),
        custom: 'created'
    }, {text: $filter('translate')('ACTIVE'), custom: 'active'}, {
        text: $filter('translate')('PROMOTED'),
        custom: 'promoted'
    }, {text: $filter('translate')('TRENDING_30'), custom: 'trending30'}, {
        text: $filter('translate')('VOTES'),
        custom: 'votes'
    }, {text: $filter('translate')('COMMENTS'), custom: 'children'}, {
        text: $filter('translate')('PAYOUT'),
        custom: 'cashout'
    }];
    ...
    $ionicModal.fromTemplateUrl('scripts/templates/story.html', {scope: $scope}).then(function (modal) {
        $scope.modalp = modal;
    });
    $scope.lastFocused;
    ...
    $rootScope.$on('insertImage', function () {
        ...
        if (type === 0) {
            ...
            remote.dialog.showOpenDialog(remote.getCurrentWindow(), {
                ...
                if (imageData) {
                    var fd = {
                        field_: 'value_',
                        file: fs.createReadStream(imageData[0])
                    };

                    ImageUploadService.uploadImage(fd).then(function (result) {
                            //var url = result.secure_url || '';
                            result = angular.fromJson(result);

                            console.log(result.imageUrl);
                            var url = result.imageUrl || '';
                            var final = " <img src='" + url + "'/>";
                            /*if ($scope.spost.body) {
            ...
            $scope.spost.body = final;
                            ...
                            editor.wrapSelection('insertHTML', final);
                            //taSelection.insertHTML(final);
                        },
                        function (err) {
    ...
    $scope.submitStory = function () {
        ...
        if ($rootScope.$storage.user) {
            ...
            if (loginSuccess) {
                ...
                angular.merge(json, {
                    tags: $scope.spost.category,
                    app: 'esteem/' + $rootScope.$storage.appversion,
                ...
                $scope.spost.body = $scope.spost.body.replaceAll('style="text-align: right;"', 'class="text-right"');
                $scope.spost.body = $scope.spost.body.replaceAll('style="text-align: left;"', 'class="text-left"');
                $scope.spost.body = $scope.spost.body.replaceAll('style="text-align: center;"', 'class="text-center"');
...
app.controller('PostCtrl', function ($scope, $stateParams, $rootScope, $interval, $ionicScrollDelegate, $ionicModal, $filter, $ionicActionSheet, $cordovaCamera, $ionicPopup, ImageUploadService, $ionicPlatform, $ionicSlideBoxDelegate, $ionicPopover, $filter, $state, textAngularManager) {
    ...
    $scope.bookmark = function () {
        ...
        if ($scope.isBookmarked()) {
            ...
            if (book) {
                $rootScope.$storage.bookmark.push({
                    title: $rootScope.$storage.sitem.title,
                    author: $rootScope.$storage.sitem.author,
                    author_reputation: $rootScope.$storage.sitem.author_reputation,
                    created: $rootScope.$storage.sitem.created,
                    permlink: $rootScope.$storage.sitem.permlink
                });
            } else {
                $rootScope.$storage.bookmark = [{
                    title: $rootScope.$storage.sitem.title,
                    author: $rootScope.$storage.sitem.author,
                    author_reputation: $rootScope.$storage.sitem.author_reputation,
                    created: $rootScope.$storage.sitem.created,
    ...
    $scope.showImages = function (index) {
        ...
        $scope.showGalleryModal('scripts/templates/gallery_images.html');
    };
        ...
        if ($scope.edit) {
            if (type === 0) {
              ...
              $scope.spost.body = final;
                                ...
                                editor = textAngularManager.retrieveEditor('editor2').scope;
                                editor.wrapSelection('insertHTML', final);
                                //taSelection.insertHTML(final);
                            },
                            function (err) {
    ...
    $ionicModal.fromTemplateUrl('scripts/templates/story.html', {
        scope: $scope
    }).then(function (modal) {
        $scope.pmodal = modal;
    });
    ...
    $scope.submitStory = function () {
        ...
        if ($scope.edit) {
            ...
            if (patch && patch.length < new Buffer($scope.spost.body, 'utf-8').length) {
                ...
                $scope.spost.body2 = $scope.spost.body2.replaceAll('style="text-align: right;"', 'class="text-right"');
                $scope.spost.body2 = $scope.spost.body2.replaceAll('style="text-align: left;"', 'class="text-left"');
                $scope.spost.body2 = $scope.spost.body2.replaceAll('style="text-align: center;"', 'class="text-center"');
        ...
        if ($rootScope.$storage.user) {
            ...
            if (loginSuccess) {
                ...
                //console.log(jjson);
                //$scope.spost.tags = $filter('lowercase')($scope.spost.tags);
                var json = angular.merge(jjson, {
                    tags: $scope.spost.tags.split(" "),
                    app: 'esteem/' + $rootScope.$storage.appversion,
                    format: 'markdown+html'
                });
                //console.log(json);
    ...
    $scope.reply = function (xx) {
        ...
        if ($rootScope.$storage.user) {
            ...
            if (loginSuccess) {
                ...
                var timeformat = t.getFullYear().toString() + (t.getMonth() + 1).toString() + t.getDate().toString() + "t" + t.getHours().toString() + t.getMinutes().toString() + t.getSeconds().toString() + t.getMilliseconds().toString() + "z";
                var json = {
                    tags: angular.fromJson($scope.post.json_metadata).tags[0] || "",
                    app: 'esteem/' + $rootScope.$storage.appversion,
                    format: 'markdown+html'
                };
                tr.add_type_operation("comment", {
    ...
    $ionicModal.fromTemplateUrl('scripts/templates/reply.html', {
        scope: $scope
    }).then(function (modal) {
        $scope.modal = modal;
    });
                ...
                confirmPopup.then(function (res) {
                    if (res) {
                        if (!$rootScope.$storage.user.password && !$rootScope.$storage.user.privateActiveKey) {
                            ...
                            if (update.profilePicUrl) {
                                delete update.profilePicUrl;
                            }
    ...
    $scope.$on('$ionicView.beforeEnter', function () {
        ...
        $scope.getOtherUsersData = function () {
            ...
            window.Api.database_api().exec("get_accounts", [[$stateParams.username]]).then(function (dd) {
                ...
                if ($rootScope.$storage.user) {
                    $scope.css = ($rootScope.$storage.user.username === $scope.user.username && $rootScope.$storage.user.json_metadata.profile.cover_image) ? {
                        'background': 'url(' + $rootScope.$storage.user.json_metadata.profile.cover_image + ')',
                        'background-size': 'cover',
                        'background-position': 'fixed'
                    } : ($rootScope.$storage.user.username !== $scope.user.username && ($scope.user.json_metadata && $scope.user.json_metadata.profile.cover_image)) ? {
                        'background': 'url(' + $scope.user.json_metadata.profile.cover_image + ')',
                        'background-size': 'cover',
                        'background-position': 'fixed'
                    } : null;
                } else {
...
app.controller('SettingsCtrl', function ($scope, $stateParams, $rootScope, $ionicHistory, $state, $ionicPopover, $ionicPopup, APIs, $filter, $translate, $window) {
    ...
    $scope.$on('$ionicView.beforeEnter', function () {
        ...
        if ($rootScope.$storage.user) {
            $rootScope.log(angular.toJson($rootScope.$storage.subscription));
            angular.merge($scope.data, {
                vote: $rootScope.$storage.subscription.vote,
                follow: $rootScope.$storage.subscription.follow,
                comment: $rootScope.$storage.subscription.comment,
                mention: $rootScope.$storage.subscription.mention,
                resteem: $rootScope.$storage.subscription.resteem
            });
            if (!$scope.$$phase) {
                $scope.$apply();
            }
